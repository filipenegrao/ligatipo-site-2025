...
export default function ThemeScrollController() {
  useEffect(() => {
    // Usa gsap.context para garantir limpeza automática
    const ctx = gsap.context(() => {
      ...
      if (best) {
        gsap.set(bg, {
          backgroundColor: best.getAttribute("data-bg") || "#fff",
        });
      ...
      if (introSection && middleLine) {
        ...
        const scrubValue =
          !isNaN(scrubAttr) && scrubAttr > 0 ? scrubAttr : true; // true follows scroll; number adds smoothing
        gsap.fromTo(
          middleLine,
          { scaleY: 1 },
          {
